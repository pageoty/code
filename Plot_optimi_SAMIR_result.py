...
# =============================================================================
    ...
    for y in ["2006","2008","2010","2012","2014","2015"]:
        ...
        Fcover_sat=Fcover_sat[["FCov","date",'ET']]
        Fcover_mod=Fcover_mod[["FCov",'date','ET']]
        #  Plot dyna Fcover
        plt.figure(figsize=(7,7))
       ...
       #  Plot dyna ETR
        ...
        ETR_FC_sat=Fcover_sat.loc[(Fcover_sat.date >= str(y)+"-03-02") &(Fcover_sat.date <= str(y)+"-10-31")]
        ETR_FC_mod=Fcover_mod.loc[(Fcover_mod.date >= str(y)+"-03-01") & (Fcover_mod.date <= str(y)+"-10-31")]
    ...
    plt.scatter(All_Fcover.FCov_sat,All_Fcover.FCov_mod)
    ...
    plt.title("ETR avec Fcover Sat/ Fcover Relation NDVI sur la période 03-02 à 08-30")
    plt.text(8,min(All_ETR.ETR_FCov_sat)+0.1,"RMSE = "+str(round(rms,2)))
    plt.text(8,min(All_ETR.ETR_FCov_sat)+0.2,"R² = "+str(round(r_value,2)))
    plt.text(8,min(All_ETR.ETR_FCov_sat)+0.3,"Pente = "+str(round(slope,2)))
    plt.text(8,min(All_ETR.ETR_FCov_sat)+0.4,"Biais = "+str(round(bias,2)))
