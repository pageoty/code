...
def params_update(path,output,date_start,date_end,FminNDVI=0.2,FmaxNDVI=0.9,FminFC=0,FmaxFC=1,Fslope=1.25,Foffset=-0.13,
                  ...
                  DiffE=0.00001,DiffR=0.00001,REW=6,minZr=125,maxZr=800,maxZr_2=800,p=0.55,FW=100,Irrig_auto=0,Irrig_man=1,Lame_max=30,minDays=20,Kcbmin_start=0.1,
...
if __name__ == "__main__":
    ...
    for y in ["2019"]:# 
        print (y)
        name_run="RUN_MULTI_SITE_ICOS/test_opti/"
        optimis_val=["Zrmax"]
        print(r'===============')
        print(optimis_val)
        print(r'===============')
        ...
        if optimis_val ==["REW"]:
            ...
            params_update(d['PC_labo']+"/Inputdata/param_SAMIR12_13.csv",
                      ...
                      Ze=125,REW=10,maxZr='optim',maxZr_2='optim',Zsoil=3000,DiffE=0.00001,DiffR=0.00001,Init_RU=0)
        ...
        if "Fcover" in name_run :
            ...
            os.system('python /home/pageot/sources/modspa2/Code/models/main/runSAMIR.py -wd /datalocal/vboxshare/THESE/BESOIN_EAU/TRAITEMENT/RUNS_SAMIR/'+name_run+'/'+str(y)+'/'' -dd /datalocal/vboxshare/THESE/BESOIN_EAU/TRAITEMENT/RUNS_SAMIR/'+name_run+'/'+str(y)+'/Inputdata/ -m meteo.df -n */NDVI'+str(y)+'.df  -fc */FC.df -wp */WP.df  -o Output/output_test -p param_modif.csv  -optim param_SAMIR12_13_optim.csv --cal ET SWC')
        
        for classe in classes:
            if len(optimis_val) < 2:
                param=pd.read_csv(d["PC_labo"]+"Output/output_test_"+classe+"_param.txt",header=None,skiprows=1,sep=";")
                param.set_index(0,inplace=True)
    
            else:
                param=pd.read_csv(d["PC_labo"]+"Output/output_test_"+classe+"_param.txt",header=None,skiprows=2,sep=";")
                param.set_index(0,inplace=True)
